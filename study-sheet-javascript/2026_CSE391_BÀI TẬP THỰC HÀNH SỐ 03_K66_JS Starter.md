...
>Dòng xuất hiện: "Hello from JavaScript!" 
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
>JavaScript đang chạy và thực thi lệnh `console.log` để in ra thông báo trong console khi trang web được tải.
...
JavaScript chạy ở cả trình duyệt và server, trình duyệt để xử lý giao diện người dùng và server thông qua Node.js để xử lý logic backend và dữ liệu
> b) HTML, CSS, JavaScript mỗi phần chịu trách nhiệm chính về điều gì?
>
> - HTML:Cấu trúc nội dung.
> - CSS: Định dạng và tạo giao diện.
> - JavaScript: Tương tác, logic, kịch bản trang web.
...
>   - `boolean` là: Nhận giá trị 0/1, True/False
...
>Hiện thị tên và tuổi của bản thân sau khi sửa thông tin
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào?
>Bỏ quên dấu `;` thì không có gì xảy ra, quên dấu `+` thì báo lỗi `Uncaught SyntaxError: missing ) after argument list` tức là quên đóng ngoặc sau dấu ".
...
>   - Tên hàm: Phần nằm trước dấu `(`, dùng để gọi hàm.
>   - Tham số (parameters): Nằm trong dấu `()`, là các biến tạm thời nhận giá trị khi gọi hàm.
>   - Thân hàm (body): Chứa các lệnh thực thi khi hàm được gọi.
>   - Giá trị trả về (return): Giá trị mà hàm trả về sau khi thực thi, được chỉ định bởi câu lệnh `return`.
> - Ưu điểm của việc dùng hàm thay vì lặp lại cùng một đoạn code nhiều lần là gì?
>   - Giúp tái sử dụng code, giảm thiểu lỗi, và làm cho chương trình dễ bảo trì hơn.
...
Lấy phần tử có id là "status" từ DOM và gán nó cho biến `statusEl`.
> - Sự kiện `"click"` xảy ra khi nào?
Khi người dùng nhấn vào nút `btnHello`.
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang?
Đoạn văn bản của phần tử `statusEl` sẽ được thay đổi thành "Xin chào! Đây là nội dung được thay đổi bằng JavaScript."
...
Nội dung: `document.body.style.backgroundColor = "lightblue";`
> - Em hãy ghi lại 1 ví dụ khác mà JavaScript có thể làm với `document.body.style`.
Ví dụ: `document.body.style.fontFamily = "Arial, sans-serif";` để thay đổi font chữ của trang.
...
Xảy ra khi người dùng nhập vào ô input, còn "click" xảy ra khi người dùng nhấn vào một phần tử nào đó.
> - Khi em xoá hết nội dung ô input, dòng `greeting` hiển thị gì?
Không có gì, chỉ hiển thị "Xin chào, !"
...
DOM là một cấu trúc dữ liệu dạng cây, trong đó mỗi phần tử HTML được biểu diễn như một nút (node) mà JavaScript có thể truy cập và thao tác để thay đổi nội dung, kiểu dáng, hoặc cấu trúc của trang web.
> - Nêu 1 ví dụ “thao tác DOM” trong bài (ghi lại 1 dòng lệnh cụ thể).
getElementById: `const statusEl = document.getElementById("status");` để lấy phần tử có id "status" từ DOM.
---
...
Ảnh chụp ở trong folder result-images.
...
Cả hai đoạn code đều lắng nghe sự kiện "click" trên phần tử có id "btnHello" và hiển thị một thông báo khi sự kiện xảy ra.
> - Điểm khác nhau về cú pháp là gì (`document.getElementById` vs `$("#id")`, `addEventListener` vs `.on`)?
document.getElementById là phương thức của JavaScript thuần để lấy phần tử theo id, trong khi $("#id") là cú pháp của jQuery để chọn phần tử. addEventListener là phương thức của JavaScript thuần để lắng nghe sự kiện, còn .on là phương thức của jQuery để làm điều tương tự.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. jQuery giúp đơn giản hóa việc thao tác DOM, xử lý sự kiện, và tạo hiệu ứng trên trang web.
>   2. jQuery cung cấp một API dễ sử dụng và tương thích với nhiều trình duyệt, giúp giảm thiểu các vấn đề về tương thích khi phát triển web.
...
Em muốn cải thiện khả năng dùng DOM để hiển thị pop-up form đăng ký.
